...
class TestInvokeClaudeCode:
    ...
    # template. The matrix below closes that gap over both transports
    # (fixed_env and passthrough_env) and both auth modes. Under
    # ``auth = "env"`` the allowlist deliberately names an UNRELATED key:
    # ``auth_env_allow`` gates the ambient-credential path, not general
    # user-declared configuration. The value is the init template's own
    # documented example.
