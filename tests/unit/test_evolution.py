...
class TestGatingInEvolutionLoop:
    ...
    def test_returns_structured_helix_result(self, mocker, tmp_path, all_mocks):
        ...
        # in the relative ordering — discovery is monotonic in accept
        # order — and compare against the live budget total.
        assert set(result.discovery_counts) == {"g0-s0", "g1-s1"}
        assert result.discovery_counts["g0-s0"] < result.discovery_counts["g1-s1"]
        assert result.discovery_counts["g1-s1"] == result.budget.evaluations
